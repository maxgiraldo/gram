...
import type { Lesson, LearningObjective, Exercise, Assessment } from '../../../types/content';

// Mock the exercise components
vi.mock('../../exercises', () => ({
  MultipleChoice: ({ question, onAnswer }: any) => (
    <div data-testid="multiple-choice">
      <button onClick={() => onAnswer({ questionId: question.id, isCorrect: true, points: 10 })}>
        Submit Answer
      </button>
    </div>
  ),
...
describe('LessonViewer', () => {
  ...
  describe('Rendering', () => {
    ...
    it('applies custom className', () => {
      const { container } = render(<LessonViewer {...defaultProps} className="custom-class" />);
  ...
  describe('Navigation', () => {
    ...
    it('calls onNavigateToSection when navigating', async () => {
      ...
      render(<LessonViewer {...defaultProps} onNavigateToSection={onNavigateToSection} />);
  ...
  describe('Progress Tracking', () => {
    it('calls onProgressUpdate when starting lesson', () => {
      ...
      render(<LessonViewer {...defaultProps} onProgressUpdate={onProgressUpdate} />);
    ...
    it('marks content section as complete when reading to end', async () => {
      ...
      const contentContainer = screen.getByText(/This is the lesson content/).closest('.prose');
      expect(contentContainer).toBeTruthy();
      
      // Mock the scroll properties to simulate reaching the end
      Object.defineProperty(contentContainer!, 'scrollTop', { value: 300, configurable: true });
      Object.defineProperty(contentContainer!, 'clientHeight', { value: 100, configurable: true });
      Object.defineProperty(contentContainer!, 'scrollHeight', { value: 300, configurable: true });
  ...
  describe('Exercise Interaction', () => {
    ...
    it('calls onExerciseAnswer when exercise is answered', async () => {
      ...
      render(<LessonViewer {...defaultProps} onExerciseAnswer={onExerciseAnswer} />);
  ...
  describe('Assessment Interaction', () => {
    ...
    it('calls onAssessmentComplete when assessment is completed', async () => {
      ...
      render(<LessonViewer {...defaultProps} onAssessmentComplete={onAssessmentComplete} />);
      ...
      await waitFor(() => {
        ...
        expect(onAssessmentComplete).toHaveBeenCalledWith('assess-1', expect.any(Number));
      });
    });
  });
  ...
  describe('Lesson Completion', () => {
    it('calls onLessonComplete when all required sections are completed', async () => {
      ...
      render(<LessonViewer {...defaultProps} onLessonComplete={onLessonComplete} />);
      ...
      const progress = {
        ...
        completedSections: ['content', 'objective-obj-1', 'exercise-ex-1', 'assessment-assess-1'],
        currentSection: 'content',
        timeSpent: 1800,
  ...
  describe('Error Handling', () => {
    ...
    it('handles exercises with no questions', async () => {
      ...
      render(<LessonViewer {...defaultProps} lesson={lessonWithEmptyExercise} />);
