...
export function AssessmentInterface({
  ...
  // Progress tracking
  useEffect(() => {
    const answered = Object.values(questionStates).filter(
      (state) => state.isAnswered
    ).length;
    const correct = Object.values(questionStates).filter(
      (state) => state.isCorrect
    ).length;
    const totalTimeSpent = Object.values(questionStates).reduce(
      (sum, state) => sum + state.timeSpent,
      0
    );
    const totalPoints = Object.values(questionStates).reduce(
      (sum, state) => sum + (state.points || 0),
      0
    );
    ...
    async (answer: ExerciseAnswer) => {
      ...
      try {
        ...
        const response = await fetch(
          `/api/assessments/attempts/${attempt.id}/responses`,
          {
            method: "POST",
  ...
  }, [announceToScreenReader, handleSubmitAssessment]);

  const canSubmit = progress.questionsAnswered === progress.totalQuestions;
  const timeDisplay =
    timer.timeLimit > 0 ? formatTime(timer.timeRemaining) : null;
  ...
  const renderCurrentQuestion = () => {
    ...
    // Adapt assessment question to exercise question format
    const adaptedQuestion = {
      ...currentQuestion,
      exerciseId: "assessment-context",
      objectiveId: currentQuestion.objectiveId || undefined,
      timeLimit: undefined,
      correctFeedback: currentQuestion.feedback || undefined,
      incorrectFeedback: currentQuestion.feedback || undefined,
      hints: currentQuestion.distractors || undefined,
    } as any;
    ...
    if (isMultipleChoiceQuestion(adaptedQuestion)) {
      return <MultipleChoice {...baseProps} />;
    }

    if (isFillInBlankQuestion(adaptedQuestion)) {
      return <FillInTheBlank {...baseProps} />;
    }

    if (isDragAndDropQuestion(adaptedQuestion)) {
      return <DragAndDrop {...baseProps} />;
    }

    if (isSentenceBuilderQuestion(adaptedQuestion)) {
  ...
  return (
    <div className={`max-w-4xl mx-auto space-y-6 ${className}`}>
      ...
      <Card>
        <CardHeader>
          ...
          <div className="mt-4">
            ...
            <ScreenReaderText>
              Progress: {progress.questionsAnswered} of{" "}
              {progress.totalQuestions} questions answered
            </ScreenReaderText>
          </div>
        </CardHeader>
      </Card>
      ...
      {/* Question content */}
      <Card>
        <CardBody>{renderCurrentQuestion()}</CardBody>
      </Card>
            ...
            <div className="flex gap-3">
              ...
              {currentQuestionIndex < questions.length - 1 ? (
                <Button
                  ...
                  disabled={
                    !questionStates[currentQuestion?.id]?.isAnswered ||
                    isSubmitting
                  }
                >
                  Next Question
