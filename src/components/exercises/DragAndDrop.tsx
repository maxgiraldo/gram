...
export function DragAndDrop({
  ...
  const handleDragEnd = useCallback(() => {
    setDraggedItem(null);
    setDraggableItems((prev) =>
      prev.map((item) => ({ ...item, isDragging: false }))
    );
    setDropTargets((prev) =>
      prev.map((target) => ({
        ...target,
  ...
  const handleDrop = useCallback(
    (e: React.DragEvent, targetId: string) => {
      ...
      const previousTarget = draggableItems.find(
        ...
        setDropTargets((prev) =>
          prev.map((target) =>
            target.id === previousTarget
              ? {
                  ...target,
                  placedItems: target.placedItems.filter(
                    (id) => id !== draggedItem
                  ),
                }
              : target
      ...
      setDraggableItems((prev) =>
        prev.map((item) =>
          item.id === draggedItem ? { ...item, isPlaced: true, targetId } : item
        )
      );
      ...
      setDropTargets((prev) =>
        prev.map((target) =>
          target.id === targetId
            ? {
                ...target,
                placedItems: [...target.placedItems, draggedItem],
                isHighlighted: false,
              }
            : target
        )
  ...
  const handleRemoveItem = useCallback(
    (itemId: string) => {
      ...
      setDraggableItems((prev) =>
        prev.map((i) =>
          i.id === itemId ? { ...i, isPlaced: false, targetId: undefined } : i
        )
      );
      ...
      setDropTargets((prev) =>
        prev.map((target) =>
          target.id === item.targetId
            ? {
                ...target,
                placedItems: target.placedItems.filter((id) => id !== itemId),
              }
            : target
        )
      );
    ...
    <Card className={`drag-and-drop-exercise ${className}`}>
      ...
      <CardFooter>
        ...
        <div className="mt-3 space-y-1 text-sm text-gray-600">
          <div>
            Items placed: {draggableItems.filter((i) => i.isPlaced).length} /{" "}
