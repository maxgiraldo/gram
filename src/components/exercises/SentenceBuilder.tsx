...
export function SentenceBuilder({
    ...
    (wordTile: WordTileState) => {
      ...
      if (wordTile.isSelected) {
        ...
        setAvailableWords((prev) =>
          [...prev, { ...wordTile, isSelected: false }].sort(
        ...
        setSelectedWords((prev) => [
          ...prev,
          { ...wordTile, isSelected: true, isPlaced: true },
        ]);
  ...
  const handleWordReorder = useCallback(
  ...
  const handleClearAll = useCallback(() => {
    ...
    const wordsToReturn = selectedWords.map((w) => ({
      ...w,
      isSelected: false,
      isPlaced: false,
    }));
    setAvailableWords((prev) =>
      [...prev, ...wordsToReturn].sort((a, b) => a.position - b.position)
    );
    setSelectedWords([]);
  ...
  const handleSubmit = useCallback(() => {
    ...
    if (validation.isCorrect) {
      setSelectedWords((prev) =>
        prev.map((w) => ({ ...w, isCorrectPosition: true }))
      );
    }
    ...
    <Card className={`sentence-builder-exercise ${className}`}>
              ...
              <div className="flex flex-wrap gap-2">
                {selectedWords.map((word, index) => (
                  <button
                    ...
                    tabIndex={
                      focusArea === "selected" && focusedWordIndex === index
                        ? 0
                        : -1
                    }
                    className={`
          ...
          >
            {availableWords.map((word, index) => (
              <button
                ...
                disabled={disabled || state.isAnswered}
                tabIndex={
                  focusArea === "available" && focusedWordIndex === index
                    ? 0
                    : -1
                }
                className={`
      ...
      <CardFooter>
        ...
        <div className="mt-3 space-y-1 text-sm text-gray-600">
          <div>
            Words used: {selectedWords.length} /{" "}
            {availableWords.length + selectedWords.length}
          </div>
          {state.hintsUsed > 0 && <div>Hints used: {state.hintsUsed}</div>}
          {timeLimit && <div>Time limit: {timeLimit} seconds</div>}
        </div>
      </CardFooter>
    </Card>
  );
